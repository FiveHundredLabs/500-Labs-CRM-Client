...
import { Contact, CallLog, Order } from '../../models/domain';
import { contactRepository, callLogRepository, orderRepository } from '../../repositories';
...
export const MemberDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const [mContacts, mLogs, mOrders] = await Promise.all([
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-blue-50/60 border border-blue-100 rounded-xl flex items-center justify-between">
              <div>
                <span className="text-[11px] font-medium uppercase tracking-wider text-blue-700">Your Rank</span>
                <div className="text-xl font-bold text-slate-900 mt-0.5">#1 in Team</div>
              </div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Track your weekly completed calls, conversion rate, and performance rank against your team members.
            </p>
            <Button
              variant="secondary"
              className="w-full"
